...
//    }
    ...
    public static long formatDoubleOngToLong(String money) {
        if (!TextUtils.isEmpty(money)) {
            BigDecimal decimalONG = new BigDecimal(money);
            BigDecimal multiplicand = new BigDecimal(Constant.ONG_TRAN);
            BigDecimal multResult = decimalONG.multiply(multiplicand);
            return multResult.longValue();
        }
        return 0;
    }

    public static String formatDoubleOngToString(String money) {
        if (!TextUtils.isEmpty(money)) {
            BigDecimal decimalONG = new BigDecimal(money);
            BigDecimal multiplicand = new BigDecimal(Constant.ONG_TRAN);
            BigDecimal multResult = decimalONG.multiply(multiplicand);
            return String.valueOf(multResult.longValue());
        }
        return "";
    }
    ...
    public static String formatONG(String amount) {
        if (!TextUtils.isEmpty(amount)) {
            BigDecimal decimalONG = new BigDecimal(amount);
            BigDecimal multiplicand = new BigDecimal(Constant.ONG_TRAN);
            BigDecimal multResult = decimalONG.divide(multiplicand, 9, BigDecimal.ROUND_HALF_UP);
